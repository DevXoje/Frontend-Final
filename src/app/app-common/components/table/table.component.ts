...
import { Component, EventEmitter, Input, OnChanges, OnInit, Output, PipeTransform, QueryList, SimpleChanges, ViewChildren } from '@angular/core';
import { FormControl } from '@angular/forms';
import { Router } from '@angular/router';
import { faCoffee, faPenToSquare, faTrash } from '@fortawesome/free-solid-svg-icons';
import { Store } from '@ngxs/store';
import { Observable, of } from 'rxjs';
import { TableElement } from '../../domain/tableElement';
import { compare, SortableHeaderDirective, SortColumn, SortEvent } from './directives/sortable.directive';
...
export class TableComponent implements OnChanges {
  ...
  onSort({ column, direction }: SortEvent) {
    this.DATA_DEFAULT = (this.DATA_DEFAULT && this.datos) ? this.datos : this.DATA_DEFAULT;
  ...
  refreshData() {
    ...
    this.DATA_DEFAULT?.subscribe(data => {
      const dataSliced = data.map((dato: any, i: number) => ({ id: i + 1, ...dato }))
